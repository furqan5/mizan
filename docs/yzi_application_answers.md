...
>
*(Submitted as written. Three statements below are superseded and are kept as the record of what was sent: the 316 chloride figure is a guidance screen rather than a measured onset and does not automatically reach a CDU plate, whose technology-cooling side never carries tower water (defect 65); the nitrate permit admits no cycle count at all on the monthly average, and 3.33 is the daily maximum alone (defect 69); and the data-centre PUE table and the +$107k / +$1.6M / +$9.8M cost of bounding came from an uncapped cubic pump law (defects 63 and 64). See `docs/defect_register.md`.)*

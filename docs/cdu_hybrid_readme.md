# Reproducing the one-CDU research extension

> **[RE-SCORED — 17 Sep 2026, branch review/cdu-side]** This experiment ran on 7–8 Sep,
> before chemistry defects 24, 25, 27, 29, 30 and 44. Its makeup carried no silica
> (`SI_silica_am` ≈ −27 at every optimum). It was re-run with thresholds untouched in two
> forms: unchanged on the current engine (`results/cdu_hybrid_rescore_20260917/`), and with
> the script's local copies of defects 25, 30 and 44 corrected by `src/cdu_hybrid_rescore.py`
...
| H1 condenser-water sign | 1610 matched pairs, fixed facility-water setpoint | FAIL; retain negative result |
| H1 cycles-to-gypsum sign | 794 matched pairs | PASS |
| H2 chemistry before equipment limits | Cycles witnesses in 8 bins; temperature witnesses in 0 bins | Cycles bind, maximum sampled FWS does not fall |
...
| A dry/wet crossover by tariff, weather, cycles | `cdu_dry_cycles.json`: 216 combinations, 65 supported | Conditional crossover computed; full-year dry ROI requires a hot-hour machine map |
...
5.8 minutes here; wall time is hardware-dependent.
...
bin records, annual results and all 909 accepted thermal
...
The independent verifier records 60 successful checks, including

...
A(quote('The evidence base is a public dataset of 147 distinct measured operating points from an experimental cooling-tower plant in Spain, published as 165 rows in three test campaigns between 2019 and 2023. We tuned the model on one campaign only and tested it on 32 points from the others that it never saw — a harder test than splitting one dataset at random. (The published third campaign turned out to repeat rows of the first, so the held-out points come from one other campaign, not two.) Six acceptance criteria were fixed and written down before the model was tuned, and the test was scored once.'));
...
A(p('**Under 300 characters:**', { after: 60 }));
A(quote('A physics model tested on 147 distinct published measurements: tuned on one test campaign, scored on 32 held-out points it never saw. Six criteria fixed in advance — three passed, three failed and are reported as failures. Water temperature predicted to 0.60 °C. Reproducible in four commands.'));
...
A(p('The evidence base is a published dataset of steady-state operation from an experimental wet cooling-tower pilot plant at Plataforma Solar de Almería — 165 rows across three campaigns between October 2019 and October 2023, 147 of them distinct operating points, MD5-verified against the published record. It was chosen over the alternatives because it carries a *measured water-consumption channel*, which lets the water model and the thermal model be validated against one consistent experiment on one rig rather than borrowing a chemistry benchmark from an unrelated plant.'));
